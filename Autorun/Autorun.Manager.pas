...
begin
  ...
  begin
    ...
    Sei.nShow := SW_HIDE;
    if Enable then
      Sei.lpParameters := LPCTSTR(TTasks.TaskNames[TTasks.TTaskType.Autorun])
    else
      Sei.lpParameters := LPCTSTR(TTasks.TaskNames[TTasks.TTaskType.DelAutorun]);

    if ShellExecuteEx(@Sei) then begin
      WaitForSingleObject(Sei.hProcess, INFINITE);

      if GetExitCodeProcess(Sei.hProcess, lpExitCode) then begin
        Result := (lpExitCode = TTasks.ERROR_Ok);
        if Result then
          if Enable then
            NotifyInfo(TLang[MsgAutorunOk]) // Программа добавлена в автозапуск.
          else
